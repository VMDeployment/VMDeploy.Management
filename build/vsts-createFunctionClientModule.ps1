...
$parentModule = 'VMDeploy.Management'
if (-not $ModuleName) { $ModuleName = 'VMDeploy.Management.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\VMDeploy.Management'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\VMDeploy.Management.psd1"
$prereqHash = @{
